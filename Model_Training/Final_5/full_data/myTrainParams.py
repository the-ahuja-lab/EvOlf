...
def trainModel(model, train_loader,
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for i in range(len(train_actual_label)):
            ...
            # key_emb.append(convert_to_list(train_key_embeddings[i][0]))
            # lock_emb.append(convert_to_list(train_lock_embeddings[i][0]))
    ...
    return model, train_list, loss_train, acc_train
...
def testModel(model, test_loader, test_text_file_path , test_key_embedding_file_path, test_lock_embedding_file_path, test_concat_embedding_file_path, testingOn = ''):
    ...
    for i in range(len(predicted_label)):
        ...
        key_emb.append(convert_to_list(test_key_embeddings[i][0]))
        lock_emb.append(convert_to_list(test_lock_embeddings[i][0]))
    ...
    writeIntoFile(test_key_embedding_file_path, f'{testingOn}.csv', key_emb)
    writeIntoFile(test_lock_embedding_file_path, f'{testingOn}.csv', key_emb)
    writeIntoFile(test_concat_embedding_file_path, f'{testingOn}.csv', key_emb)
