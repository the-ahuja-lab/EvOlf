...
def getIds(dataType, fold_num, makeFolds = False):
    ...
    if dataType == 'evolf':
        main = pd.read_csv('../raw_data/TrainDataRaw/25_EvOlf_MainDataset.csv')
    elif dataType == 'glass':
        main = pd.read_csv('../raw_data/TrainDataRaw/25_GLASS_MainDataset.csv')
    ...
    if makeFolds:
        save_path = f'../processedData/10Fold/fold_{fold_num}/ids/'
    else:
        save_path = f'../processedData/ids/'
...
def processTrainData(dataType, dataSubtype, fold_num, makeFolds):
    ...
    if makeFolds:
        ...
        save_path = f'../processedData/SingleDataset/ids/'
    ...
    if makeFolds:
        scaler_path = f'../processingModels/10Folds/fold_{fold_num}/scaler_models'
    else:
        scaler_path = '../processingModels/scaler_models'
    ...
    if makeFolds:
        pca_path = f'../processingModels/10Folds/fold_{fold_num}/pca_models'
    else:
        pca_path = '../processingModels/pca_models'
    ...
    if makeFolds:
        file_path = f'../raw_data/TrainDataUsable/10Fold/fold_{fold_num}/'
    else:
        file_path = f'../raw_data/TrainDataUsable/'
    ...
    if makeFolds:
        save_path = f'../processedData/10Fold/{dataType}/{dataSubtype}/{fold_num}/'
    else:
        save_path = f'../processedData/{dataType}/{dataSubtype}/'
...
def processTestData(mode, dataType, dataSubtype, processDataType, processDataSubtype, fold_num, makeFolds):
    # dataType: testing data that we want to process, e.g. glass
    # subtype: all, human
    # processDataType: model that we are training, e.g. evolf
    ...
    if makeFolds:
        save_path = f'../processedData/10Fold/{processDataType}/{processDataSubtype}/{fold_num}/'
    else:
        save_path = f'../processedData/{processDataType}/{processDataSubtype}/'
