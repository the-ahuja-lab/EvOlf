...
class MyModel(nn.Module):
  ...
  def __init__(self, input_dim=128, hidden_dim=128, num_heads=4, num_layers=2):
    ...
    self.dropout = nn.Dropout(0.2) # mention
    # feed forward - input moving forward through linear layers
  ...
  def forward(self, k1, k2, k3, k4, k5, l1, l2, l3, l4):
    ...
    key_ft = torch.cat([k1, k2, k3, k4, k5], dim=1) # concat all keys
    key_ft = self.key_transformer(key_ft, key_ft) # self-attention - key
    key_ft_norm = self.layer_norm1(key_ft)  #  layer normalization
    key_ft = self.key_cnn(key_ft + key_ft_norm) # transformer output + normalization output
    key_ft, (h_k, c_k) = self.key_lstm(key_ft)
    key_ft_final = h_k[-1] # pick the last layer, total 10 indices

    lock_ft = torch.cat([l1, l2, l3, l4], dim=1)
    lock_ft = self.lock_transformer(lock_ft, lock_ft) # self-attention - lock
    lock_ft_norm = self.layer_norm2(lock_ft)  
    lock_ft = self.lock_cnn(lock_ft + lock_ft_norm)
    lock_ft, (h_l, c_l) = self.lock_lstm(lock_ft)
    lock_ft_final = h_l[-1]

    concat_final = self.fusion_transformer(key_ft_final.reshape(key_ft_final.shape[0], 1, key_ft_final.shape[1]), lock_ft_final.reshape(lock_ft_final.shape[0], 1, lock_ft_final.shape[1])) # cross or self attention, make it 3d again
