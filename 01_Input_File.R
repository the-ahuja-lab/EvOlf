...
dataPath <- "/storage1/aayushim/EvOlf_3.3/01_Dry_Lab/15_Colab/Sample_Run_01/01_SampleInput/01_SampleInputData_IDs.csv"
tempPath <- "/storage1/aayushim/EvOlf_3.3/01_Dry_Lab/15_Colab/Sample_Run_01/Temp_Folder"

ligSmilesColumn <- "SMILES"
recSeqColumn <- "Sequence"
ligID <- NULL
recID <- NULL
lrID <- NULL

# take input file from user
mainData <- read.csv(dataPath)
...
write.csv(mainData, file = paste0(tempPath, "/mainData_01.csv"), row.names = FALSE)
write.csv(ligsData, file = paste0(tempPath, "/ligsData.csv"), row.names = FALSE)
write.csv(recsData, file = paste0(tempPath, "/recsData.csv"), row.names = FALSE)

# make fasta file for receptor

# function to create fasta files:
dat2fasta <- function(id, sequence, outfile = paste0(tempPath, "/recsData.fasta")){
